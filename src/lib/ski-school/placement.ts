...
export function rosterValidationError(
  ...
  for (const sid of g.studentIds) {
    ...
    if (!studentFitsGroup(s, g)) {
      return `${formatFullName(s)} doesn’t match this lesson’s age range or discipline.`;
    }
    if (studentHasOverlappingAssignment(groups, sid, g.day, g.time, g.id)) {
      return `${formatFullName(s)} is already in another lesson at an overlapping time.`;
    }
  }
  for (const iid of g.instructorIds) {
    ...
    if (!instructorFitsGroup(ins, g)) {
      return `${formatFullName(ins)} doesn’t teach this lesson’s discipline.`;
    }
    if (instructorHasOverlappingAssignment(groups, iid, g.day, g.time, g.id)) {
      return `${formatFullName(ins)} is already in another lesson at an overlapping time.`;
    }
  }
  return null;
}
